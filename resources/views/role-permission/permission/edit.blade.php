        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form class="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md" action="{{ url('permissions/' . $permission->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <h2 class="text-2xl font-bold text-center mb-6">Edit Permission</h2>

                        <div class="mb-4">
                            <label for="name" class="block text-gray-700 font-bold mb-2">Permission Name:</label>
                            <input type="text" id="name" name="name" value="{{ $permission->name }}" class="w-full px-3 py-2 border rounded-lg" required>
                        </div>
...
{{--                         <div class="mb-4">
                        ...
                        <button type="submit" href="{{ url('permissions') }}" class="w-full bg-blue-500 text-white font-bold py-2 rounded-lg hover:bg-blue-700">Update</button>
                    </form>                   
                </div>
            </div>
        </div>
